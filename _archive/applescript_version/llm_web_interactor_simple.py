...
def build_typing_simulation(message):
    ...
    return f"""
        ...
        const textToType = "{escaped_message}";
...
def build_response_capture(capture_html: bool = False):
    """Generate JavaScript for capturing the response, optionally including HTML."""
    return f"""
        const conversation = {{
            conversation: [],
        ...
        {f"conversation.html = document.documentElement.outerHTML;" if capture_html else ""}
...
def assemble_applescript_send_message(message, url="https://chat.qwen.ai/"):
    ...
    """
    logger.debug(f"Generated AppleScript for sending message:\n{script}")
    return script


def assemble_applescript_capture_response(
    url="https://chat.qwen.ai/", capture_html: bool = False
):
    """Assemble AppleScript for capturing the response, optionally including HTML."""
    script = f"""
    tell application "Google Chrome"
        activate
        {build_tab_selection(url)}
        tell active tab of front window
            execute javascript "
                (function() {{
                    {build_response_capture(capture_html)}
                }})();
            "
        end tell
    end tell
    """
    logger.debug(f"Generated AppleScript for capturing response:\n{script}")
    return script
...
def ask_llm_question(
    ...
    """
    Send a question to the LLM and retrieve the response, optionally capturing HTML.
    """
    try:
        ...
        capture_script = assemble_applescript_capture_response(url, capture_html)
